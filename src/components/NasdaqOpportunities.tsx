...
import { FaCalculator, FaMicrochip, FaInfoCircle, FaRocket } from 'react-icons/fa';
import { analyzeNasdaqOpportunities, calculateNasdaqProjection } from '../services/nasdaqAnalyzer';
import { formatCurrency } from '../utils/formatters';
import { Tooltip } from 'react-tooltip';

const NasdaqOpportunities: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOpportunities = async () => {
      try {
        const data = await analyzeNasdaqOpportunities();
  ...
  const handleCalculateProjection = () => {
    ...
    const results = calculateNasdaqProjection(opportunities, amount);
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <FaMicrochip className="text-blue-600" size={24} />
            <h2 className="text-2xl font-bold">Top Tech Opportunities</h2>
          </div>
          <FaInfoCircle 
            className="text-gray-400 cursor-help"
            data-tooltip-id="nasdaq-info"
          />
        </div>
          ...
          {Array.isArray(opportunities) && opportunities.map((opportunity) => (
            ...
            >
              ...
              <div className="mt-4 grid grid-cols-4 gap-4">
                ...
                <div>
                  <div className="text-sm text-gray-600">Score Croissance</div>
                  <div className="font-semibold">{opportunity.growthScore}%</div>
                </div>
                <div>
                  <div className="text-sm text-gray-600">Score Innovation</div>
                  <div className="font-semibold">{opportunity.innovationScore}%</div>
                </div>
                <div>
                  <div className="text-sm text-gray-600">Potentiel mensuel</div>
                  <div className="font-semibold text-green-600">
                    +{opportunity.monthlyUpside.toFixed(2)}%
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 mt-4">
                <div>
                  ...
                  <ul className="text-sm text-gray-600 space-y-1">
                    {opportunity.reasons.map((reason: string, index: number) => (
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="mt-4 text-sm text-gray-500">
              * Les projections sont basées sur une période d'un mois et sont données à titre indicatif.
              Le marché tech est particulièrement volatil.
            </div>
          </div>
        )}
      </div>

      <Tooltip id="nasdaq-info">
        <div className="max-w-xs">
          <p className="font-bold mb-1">Analyse NASDAQ</p>
          <p>
            Notre algorithme analyse en continu le marché tech pour identifier les meilleures
            opportunités d'investissement basées sur des critères techniques et fondamentaux.
          </p>
        </div>
      </Tooltip>
    </div>
...
export default NasdaqOpportunities;